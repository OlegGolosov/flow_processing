...
struct ResolutionTrack {
  ...
  ResolutionTrack (float _ptMin, float _ptMax, float _yMin, float _yMax, float _scale = 1.) :
	  ptMin(_ptMin), ptMax(_ptMax), yMin(_yMin), yMax(_yMax), scale(_scale) {};
  ResolutionTrack (std::vector <float> prop, float _scale = 1.) :
	  ptMin(prop.at(0)), ptMax(prop.at(1)), yMin(prop.at(2)), yMax(prop.at(3)), scale(_scale) {};
  ...
  Result operator()(std::vector<Result> args, const vector<string> &argNames) const {
    ...
    if (args.size() == 3)
    {
      ...
      nom1 = Rebin(nom1, "Pt", {ptMin, ptMax});
      nom1 = Rebin(nom1, "Rapidity", {yMin, yMax});
      nom1 = nom1.Projection({"Centrality"});
      Result nom2 = args[1];
      nom2 = SetReference(nom2);
      nom2 = Rebin(nom2, "Pt", {ptMin, ptMax});
      nom2 = Rebin(nom2, "Rapidity", {yMin, yMax});
      nom2 = nom2.Projection({"Centrality"});
      Result denom = args[2];
      result = Qn::Sqrt(nom1 * nom2 / denom);
    }
    else if (args.size() == 2)
...
struct Resolution4S {
  ...
  Resolution4S (float _ptMin, float _ptMax, float _yMin, float _yMax, float _scale = 1.) :
	  ptMin(_ptMin), ptMax(_ptMax), yMin(_yMin), yMax(_yMax), scale(_scale) {};
    
  Resolution4S (vector<float> props, float _scale = 1.) :
	  ptMin(props.at(0)), ptMax(props.at(1)), yMin(props.at(2)), yMax(props.at(3)), scale(_scale) {};
  ...
  Result operator()(vector<Result> args, const vector<string> &) {
    ...
    if (args.size() == 3) {
      ...
      denom = Rebin(denom, "Pt", {ptMin, ptMax});
      denom = Rebin(denom, "Rapidity", {yMin, yMax});
      denom = denom.Projection({"Centrality"});
      result = nom * RT / denom;
    } else if (args.size() == 2) {
      Result nom = args[0];
      Result RT = args[1];
      nom = SetReference(nom);
      nom = Rebin(nom, "Pt", {ptMin, ptMax});
      nom = Rebin(nom, "Rapidity", {yMin, yMax});
      nom = nom.Projection({"Centrality"});
      result = nom / RT;
    } else if (args.size() == 1) {
      Result nom = args[0];
      nom = SetReference(nom);
...
struct ResolutionMH {
  ...
  ResolutionMH (float _ptMin, float _ptMax, float _yMin, float _yMax) :
	  ptMin(_ptMin), ptMax(_ptMax), yMin(_yMin), yMax(_yMax) {};
  ResolutionMH (std::vector<float> prop) :
	  ptMin(prop.at(0)), ptMax(prop.at(1)), yMin(prop.at(2)), yMax(prop.at(3)) {};
...
struct FlowV1 
{
  vector <string> projectionAxes;
  vector <string> rebinAxes;
  vector <float> cuts;
  FlowV1 (const vector<string> _projectionAxes, const vector<string> &_rebinAxes, const vector<float> &_cuts) :
	  projectionAxes(_projectionAxes), rebinAxes(_rebinAxes), cuts(_cuts) {};
  Result operator()(std::vector<Result> args, const vector<string> &argNames) const 
  {
    auto uQ = args.at(0);
    for (int iAxis=0; iAxis < rebinAxes.size(); iAxis++)
    {
      string axisName = rebinAxes.at(iAxis);
      float min = cuts.at(iAxis*2);
      float max = cuts.at(iAxis*2 + 1);
      uQ = Rebin(uQ, axisName, {min, max});
    }
    uQ=Projection(uQ, projectionAxes);
...
int process_2d(
  ...
  /** Resolution PSD **/
  {
    ...
    std::map <const char*, vector<float>> detProp4S = {
      //{"proton",   {0., 2., -0.6, -0.2,  1., -1.}},
      {"pion_pos", { 0., 2.2, 0.8,  1.2,  1., -1.}},
      //{"pion_neg", { 0., 2., 0.8,  1.2,  -1., 1.}},
      //{"mcPid_proton",   {0., 2., -0.6, -0.2,  -1., 1.}},
      //{"mcPid_pion_pos", { 0., 2.2, 0.8,  1.2,  -1., 1.}},
      //{"mcPid_pion_neg", { 0., 2., 0.8,  1.2,  -1., 1.}},
    };
    for (auto props:detProp4S)
    {
      ...
      for (auto &comp:{"XX","YY","XY","YX"})
      {
        ...
        {
          rm.Define(Form ("%s_res_%s_%s", particle, psd, comp), {Form ("%s_%s_%s", particle, psd, comp)}, ResolutionTrack{prop});
          rm.Define(Form ("%s_res_psi_%s_psi_%s", particle, psd, comp), {Form("%s_psi_%s", particle, comp), Form("%s_psi_%s", psd, comp)}, ResolutionTrack{prop});
        }
      }
      rm.Define(Form("RES_%s_MC_X",particle), {Form("%s_psi_XX", particle)}, ResolutionTrack{prop, prop.at(5)});
      rm.Define(Form("RES_%s_MC_Y",particle), {Form("%s_psi_YY", particle)}, ResolutionTrack{prop, prop.at(5)});
      rm.Define(Form("RES_%s_psd1_psd3_X", particle), {Form("%s_psd1_XX", particle), Form("%s_psd3_XX", particle), "psd3_psd1_XX"}, ResolutionTrack{prop});
      rm.Define(Form("RES_%s_psd1_psd3_Y", particle), {Form("%s_psd1_YY", particle), Form("%s_psd3_YY", particle), "psd3_psd1_YY"}, ResolutionTrack{prop});
      rm.Define(Form("RES_psd1_%s_4S_X", particle), {"psd3_psd1_XX", Form("RES_%s_psd1_psd3_X", particle), Form("%s_psd3_XX", particle)}, Resolution4S{prop, prop.at(5)});
      rm.Define(Form("RES_psd2_%s_4S_X", particle), {Form("%s_psd2_XX", particle), Form("RES_%s_psd1_psd3_X", particle)}, Resolution4S{prop, prop.at(5)});
      rm.Define(Form("RES_psd3_%s_4S_X", particle), {"psd3_psd1_XX", Form("RES_%s_psd1_psd3_X", particle), Form("%s_psd1_XX", particle)}, Resolution4S{prop, prop.at(5)});
      rm.Define(Form("RES_psd1_%s_4S_Y", particle), {"psd3_psd1_YY", Form("RES_%s_psd1_psd3_Y", particle), Form("%s_psd3_YY", particle)}, Resolution4S{prop, prop.at(5)});
      rm.Define(Form("RES_psd2_%s_4S_Y", particle), {Form("%s_psd2_YY", particle), Form("RES_%s_psd1_psd3_Y", particle)}, Resolution4S{prop, prop.at(5)});
      rm.Define(Form("RES_psd3_%s_4S_Y", particle), {"psd3_psd1_YY", Form("RES_%s_psd1_psd3_Y", particle), Form("%s_psd1_YY", particle)}, Resolution4S{prop, prop.at(5)});
    }
    ...
    //    {"proton",   {0., 3., -0.4, 0.8, 1.}},
    //    {"pion_pos", {0., 3., -0.4, 0.8, 1.}},
    //    //{"pion_neg", {0., 3., -0.4, 0.8, 1.}},
    //  };
    ...
    //    {
    //      rm.Define(Form ("%s_psd1_psd2_%s", particle, comp), {Form ("%s_psd1_psd2_%s", particle, comp)}, ResolutionMH{prop});
    //      rm.Define(Form ("%s_psd2_psd3_%s", particle, comp), {Form ("%s_psd2_psd3_%s", particle, comp)}, ResolutionMH{prop});
    //    }
    //    rm.Define(Form("RES_psd1_%s_MH_X1", particle),
    //              {Form("%s_psd1_psd2_X2XX", particle), "psd3_psd1_XX", Form("%s_psd2_psd3_X2XX", particle)},
    //              ResolutionMH{prop});
    //    rm.Define(Form("RES_psd1_%s_MH_X2", particle),
    //              {Form("%s_psd1_psd2_Y2XY", particle), "psd3_psd1_XX", Form("%s_psd2_psd3_Y2YX", particle)},
    //              ResolutionMH{prop});
    ...
    //              {Form("%s_psd1_psd2_X2YY", particle), "psd3_psd1_YY", Form("%s_psd2_psd3_Y2XY", particle)},
    //              ResolutionMH{prop});
    //    rm.Define(Form("RES_psd1_%s_MH_Y2", particle),
    //              {Form("%s_psd1_psd2_Y2YX", particle), "psd3_psd1_YY", Form("%s_psd2_psd3_Y2XY", particle)},
    //              ResolutionMH{prop});
    ...
    //              {Form("%s_psd2_psd3_X2XX", particle), "psd3_psd1_XX", Form("%s_psd1_psd2_X2XX", particle)},
    //              ResolutionMH{prop});
    //    rm.Define(Form("RES_psd3_%s_MH_X2", particle),
    //              {Form("%s_psd2_psd3_Y2YX", particle), "psd3_psd1_XX", Form("%s_psd1_psd2_Y2XY", particle)},
    //              ResolutionMH{prop});
    ...
    //              {Form("%s_psd2_psd3_Y2XY", particle), "psd3_psd1_YY", Form("%s_psd1_psd2_Y2YX", particle)},
    //              ResolutionMH{prop});
    //    rm.Define(Form("RES_psd3_%s_MH_Y2", particle),
    //              {Form("%s_psd2_psd3_X2YY", particle), "psd3_psd1_YY", Form("%s_psd1_psd2_X2YY", particle)},
    //              ResolutionMH{prop});
  ...
  ///** V1 **/
  {
    vector < tuple < const char*, const char*, vector<string>, vector<string>, vector <float> > > partProps = {
      //{"proton", "b_pT",  {"Centrality",       "Pt"}, {"Rapidity"}, { -0.4, -0.2}},
      //{"proton", "f_pT",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.2,  0.4}},
      //{"proton",    "y",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.6,  1.2}},
      //{"pion_pos", "pT",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.6,  0.8}},
      //{"pion_pos",  "y",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.6,  1.0}},
      //{"pion_neg", "pT",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.6,  0.8}},
      //{"pion_neg",  "y",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.6,  1.0}},
      //{"kaon_pos", "pT",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.2,  0.4}},
      //{"kaon_pos",  "y",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.2,  0.6}},
      //{"kaon_neg", "pT",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.2,  0.4}},
      //{"kaon_neg",  "y",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.2,  0.6}},
      {"deutron", "pT_0",  {"Centrality",       "Pt"}, {"Rapidity"}, { -0.8, -0.6}},
      {"deutron", "pT_1",  {"Centrality",       "Pt"}, {"Rapidity"}, { -0.6, -0.2}},
      {"deutron", "pT_2",  {"Centrality",       "Pt"}, {"Rapidity"}, { -0.2,  0.2}},
      {"deutron", "pT_3",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.2,  0.6}},
      {"deutron", "pT_4",  {"Centrality",       "Pt"}, {"Rapidity"}, {  0.6,  0.8}},
      {"deutron",  "y_0",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.0,  0.4}},
      {"deutron",  "y_1",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.4,  0.8}},
      {"deutron",  "y_2",  {"Centrality", "Rapidity"},       {"Pt"}, {  0.8,  1.2}},
      {"deutron",  "y_3",  {"Centrality", "Rapidity"},       {"Pt"}, {  1.2,  1.8}},
      {"deutron",  "y_4",  {"Centrality", "Rapidity"},       {"Pt"}, {  1.8,  2.6}},
    };
    vector <string> pidSources{"", "mc_", "mcPid_"};
    std::vector<std::string> psd_references{"psd1", "psd2", "psd3", "(psd1|psd2)", "(psd1|psd2|psd3)"};
    std::vector<std::string> components{"XX", "YY", "(X|Y)"};
    std::vector<std::string> resolution_methods{"MC", "pion_pos_4S", /*"pion_neg_4S", "proton_4S", "proton_MH", "pion_neg_MH", "pion_pos_MH", "proton_4S", "pion_neg_4S", "pion_pos_4S", "proton_MH", "pion_neg_MH", "pion_pos_MH", "3S"*/};

    
    // mc_proton_psi_XX
    const char *uQ_psi_pattern = "%s%s_psi_%s";

    // v1_proton_y_psi_X
    const char *v1_psi_pattern = "v1_%s%s_%s_psi_%s";
    
    // proton_psd1_XX
    const char *uQ_pattern = "%s%s_%s_%s";
    
    // v1_proton_y_psd1_3S_X
    const char *v1_pattern = "v1_%s%s_%s_%s_%s_%s";

    // RES_psd1_3S_X
    const char *R_pattern = "RES_%s_%s_%s";

    for (const auto &partProp : partProps)
    {
      auto &particle = get<0>(partProp);
      auto &axis = get<1>(partProp);
      auto &projectionAxes = get<2>(partProp);
      auto &rebinAxes = get<3>(partProp);
      auto &cuts = get<4>(partProp);
      bool combineComponents, combineReferences;
      
      for (auto &pid:pidSources)
      {
        for (const auto &component : components) 
        {
          ...
          std::string v1_psi_name{sprintf(v1_psi_pattern, pid, particle, axis, component_out)};
          if (!combineComponents)
          {
            std::string uQ_psi_name{sprintf(uQ_psi_pattern, pid, particle, component)};
            rm.Define(v1_psi_name, {uQ_psi_name}, FlowV1{projectionAxes, rebinAxes, cuts});
          }
          else
          {
            std::string v1_psi_name_pattern{sprintf(v1_psi_pattern, pid, particle, axis, component)};
          ...
          for (const auto &psd_reference : psd_references)
          {
            ...
            {
              std::string v1_name{sprintf(v1_pattern, pid, particle, axis, psd_reference_out, res_method, component_out)};
              if (!combineComponents && !combineReferences)
              {
                std::string uQ_name{sprintf(uQ_pattern, pid, particle, psd_reference, component)};
                std::string R_name{sprintf(R_pattern, psd_reference, res_method, component_out)};
                rm.Define(v1_name, {uQ_name, R_name}, FlowV1{projectionAxes, rebinAxes, cuts});
              }
              else
              {
                ...
                if (combineComponents)
                  comp = component;
                std::string v1_name_pattern{sprintf(v1_pattern, pid, particle, axis, psd_reference, res_method, comp)};
